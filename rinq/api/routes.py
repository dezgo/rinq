...
def _get_call_state_inner(agent_call_sid, caller_email=None):
    ...
    for u in all_users:
        ...
        if email:
            identity = _email_to_browser_identity(email)
    ...
    # Pre-lookup: gather transfer consult call SIDs for name resolution
    _transfer_call_names = {}  # consult_call_sid -> target_name

    def resolve_participant(call_sid):
        """Resolve a call SID to name and role."""
        # If this is the requesting agent's own call, use their session info
        ...
        # Check if this is a known transfer consult call
        if call_sid in _transfer_call_names:
            return {'call_sid': call_sid, 'name': _transfer_call_names[call_sid], 'role': 'transfer_target'}

        # Check queued_calls (customer)
        ...
        # Check call_log
        agent_email = db.get_call_log_field(call_sid, 'agent_email')
        if agent_email:
            email, friendly = _normalize_staff_identifier(agent_email)
            if friendly:
        ...
        try:
            ...
            for identifier in [call.to, call.from_]:
                if identifier and identifier in user_map:
                    return {'call_sid': call_sid, 'name': user_map[identifier]['name'], 'role': 'agent'}
                if identifier:
    ...
    # Find the agent's conference by looking at queued_calls
    # The agent answered a queue call → customer SID is in queued_calls with conference_name
    result = {
        'in_call': True,
        'conference': None,
        'participants': [],
    ...
    # Strategy 1: Look for a queued call that was answered and has this conference
    # The conference name pattern is hold_room_{customer_call_sid}
    # We need to find which customer this agent is talking to
    try:
        # Check all in-progress conferences for this agent's call SID
        # This is expensive but reliable
        in_progress_calls = twilio_service.client.calls(agent_call_sid).fetch()
        # If the agent is in a conference, the call's status will be in-progress
        # and we can check conferences
    except Exception:
        return jsonify({"in_call": False})

    # Find conference containing this agent
    # Look at recent queued calls that are answered
            ...
            if agent_in_conf:
                ...
                if transfer_state and transfer_state.get('transfer_status') in ('pending', 'consulting'):
                    ...
                    if consult_sid and target_name:
                        _transfer_call_names[consult_sid] = target_name
                    ...
                    if consult_conf:
                        consult_parts = get_conference_state(consult_conf)
                        if consult_parts:
        ...
        for qc in answered_calls:
            ...
            try:
                ...
                if agent_in_conf:
                    ...
                    # Agent is in the consult conference
                    result['conference'] = qc.get('conference_name')
                    result['customer_call_sid'] = qc.get('call_sid')

                    # Original conference participants
                    orig_parts = get_conference_state(qc.get('conference_name'))
...
def get_conference_participants():
    ...
    # Build a quick SIP username -> email map for resolution
    all_users = db.get_users()
    user_map = {}
    for u in all_users:
        email = u.get('staff_email', '')
        username = u.get('username', '')
        if email:
            # Map browser identity to name
            identity = _email_to_browser_identity(email)
            friendly = email.split('@')[0].replace('.', ' ').replace('_', ' ').title()
            user_map[f"client:{identity}"] = friendly
            if username:
    ...
    try:
        ...
        for p in participants:
            ...
            # Check if it's the customer (queued call)
            queued = db.get_queued_call_by_sid(call_sid)
            if queued:
                name = queued.get('customer_name') or queued.get('caller_number', 'Customer')
                role = 'customer'
            else:
                # Check call_log for agent info
                agent_email = db.get_call_log_field(call_sid, 'agent_email')
                if agent_email:
                    email, friendly = _normalize_staff_identifier(agent_email)
                    name = friendly or (email.split('@')[0].replace('.', ' ').title() if email else None)
                    role = 'agent'

                if not name:
                    # Resolve from Twilio call's to/from fields
                    try:
                        call = twilio_service.client.calls(call_sid).fetch()
                        for identifier in [call.to, call.from_]:
                            if identifier and identifier in user_map:
                                name = user_map[identifier]
                                role = 'agent'
                                break
                            if identifier:
                                email, friendly = _normalize_staff_identifier(identifier)
        ...
        logger.warning(f"Error getting conference participants: {e}")
        return jsonify({"participants": []})
...
@api_bp.route('/voice/transfer/targets', methods=['GET'])
@api_or_session_auth
def get_transfer_targets():
    ...
    Returns:
        {
            "targets": [
                {"email": "...", "name": "...", "phone": "...", "has_browser": true/false},
                ...
            ]
        }
    """
...
@api_bp.route('/voice/transfer/blind', methods=['POST'])
@api_or_session_auth
def blind_transfer():
    """Execute a blind (cold) transfer.

    Immediately transfers the caller to the target. The agent is disconnected.

    Request body:
        call_sid: The original caller's call SID
        target: Phone number to transfer to
        target_name: Display name for the target (for logging)

    Returns:
        {"success": true, "transfer_type": "blind"} or {"error": "..."}
    """
    from rinq.services.transfer_service import get_transfer_service
...
@api_bp.route('/voice/transfer/blind-direct', methods=['POST'])
@api_or_session_auth
def blind_transfer_direct():
    """Execute a blind transfer on a direct (non-conference) call.

    Used for outbound calls made via the softphone where there's no
    conference involved.
    ...
    Request body:
        ...
        target: Phone number to transfer to
        target_name: Display name for the target (for logging)
        caller_id: Caller ID to use for the outbound leg (optional)
    ...
    conference_name = db.get_call_conference(call_sid)
    if conference_name:
        # Find the customer's call SID from the conference
        child_sid = db.get_call_child_sid(call_sid)
        if child_sid:
...
def warm_transfer_start():
    ...
    # For queue calls, conference is stored against the customer SID
    if not conf_name and call_sid:
        conf_name = db.get_call_conference(call_sid)
    # Customer SID: from child_sid (outbound) or call_sid (queue)
    customer_sid = child_sid or call_sid

    if three_way and conf_name and customer_sid:
        # 3-way: just add target to existing conference — no hold, no redirect
        ...
        # Warm transfer: conference-first path
        ...
        # Fallback for non-conference calls
...
@api_bp.route('/voice/transfer/warm/complete', methods=['POST'])
@api_or_session_auth
def warm_transfer_complete():
    ...
    Request body:
        call_sid: The original caller's call SID (queue calls)
        transfer_key: The transfer key returned from warm/start
        call_type: 'queue', 'outbound', or 'direct_inbound'
    ...
    # Try conference-based complete first (works for queue AND conference-first calls)
    ...
    if not result.get('success') and result.get('error') == 'No transfer in progress':
        # Fallback to universal for old-style non-conference calls
...
@api_bp.route('/voice/transfer/cancel', methods=['POST'])
@api_or_session_auth
def transfer_cancel():
    ...
    # Try conference-based cancel first (works for queue AND conference-first calls)
...
@api_bp.route('/voice/transfer/status', methods=['GET'])
@api_or_session_auth
def get_transfer_status():
    ...
    Query params:
        call_sid: The original caller's call SID (queue calls)
        transfer_key: The transfer key (non-queue calls)
        source: 'queued_calls' or 'call_log'

    Returns:
        {"transfer": {...}} or {"transfer": null}
    """
    db = get_db()
...
@api_bp.route('/voice/transfer/consult-join', methods=['POST'])
def transfer_consult_join():
    """TwiML webhook when transfer target answers consultation call.

    Joins the target to the consultation conference with the agent.
...
<Response><Say>Sorry, an error occurred.</Say><Hangup/></Response>''',
    ...
    # Join the consultation conference
    twiml = f'''<?xml version="1.0" encoding="UTF-8"?>
<Response>
...
@api_bp.route('/voice/transfer/agent-consult', methods=['POST'])
def transfer_agent_consult():
    """TwiML webhook to move agent to consultation conference.
...
<Response><Say>Sorry, an error occurred.</Say><Hangup/></Response>''',
    ...
    # Join the consultation conference
    # Agent starts it and can end it
    twiml = f'''<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Dial>
...
@api_bp.route('/voice/transfer/target-join', methods=['POST'])
def transfer_target_join():
    """TwiML webhook to move transfer target to original conference.

    Called when completing a warm transfer - the target leaves the consultation
    conference and joins the original conference with the caller.
...
<Response><Say>Sorry, an error occurred.</Say><Hangup/></Response>''',
    ...
    # Join the original conference with the caller
    # The target takes over the agent role
    twiml = f'''<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Dial>
        <Conference beep="false" startConferenceOnEnter="true" endConferenceOnExit="true">
...
def transfer_consult_status():
    ...
    Handles cases where the transfer target doesn't answer or the call fails.
...
@api_bp.route('/voice/recording/start', methods=['POST'])
@api_or_session_auth
def start_recording():
    ...
    Returns:
        {"success": true, "recording_sid": "..."} or {"error": "..."}
    """
...
@api_bp.route('/voice/recording/stop', methods=['POST'])
@api_or_session_auth
def stop_recording():
    ...
    Returns:
        {"success": true, "stopped_count": N} or {"error": "..."}
    """
    from rinq.services.recording_service import recording_service
...
@api_bp.route('/voice/recording/status', methods=['GET'])
@api_or_session_auth
def get_recording_status():
    ...
    Returns:
        {"recording": true/false, "recording_sid": "..."} or {"error": "..."}
    """
    from rinq.services.recording_service import recording_service
    ...
    result = recording_service.get_recording_status(call_sid)
    return jsonify(result)


@api_bp.route('/voice/recording-status', methods=['POST'])
def recording_status_webhook():
    """Handle recording status webhook from Twilio.

    Twilio calls this when a recording finishes processing.
    We download the recording, email it to Google Group, and log it.
    ...
    # Check if this recording was already processed (e.g., by voicemail handler)
    db = get_db()
    ...
    # Fetch call details from Twilio (webhook doesn't include From/To)
    from_number = ''
    to_number = ''
    try:
        twilio = get_twilio_service()
        ...
        logger.info(f"Fetched call details: {from_number} -> {to_number}")
    except Exception as e:
        logger.warning(f"Could not fetch call details from Twilio: {e}")

    # Look up call info to determine call type and staff
    call_type = 'unknown'
    staff_email = None
    staff_name = None
    caller_name = None  # Customer name from Clara lookup

    # Try to get info from queued_calls (if this was an inbound queue call)
    queued_call = db.get_queued_call_by_sid(call_sid)
    if queued_call:
        call_type = 'inbound'
        answered_by = queued_call.get('answered_by')
        from_number = from_number or queued_call.get('caller_number', '')
        caller_name = queued_call.get('customer_name')  # From Clara enrichment
        # Normalize the answered_by field (handles client:, session:, emails, SIP URIs)
        staff_email, staff_name = _normalize_staff_identifier(answered_by)
    else:
        # Check activity log to determine call type and staff
...
@api_bp.route('/users/me/recording-default', methods=['GET'])
@login_required
...
@api_bp.route('/users/me/recording-default', methods=['PUT'])
@login_required
def set_my_recording_default():
    ...
    Returns:
        {"success": true, "recording_enabled": true/false}
    """
    from rinq.services.recording_service import recording_service
...
def list_recordings():
    ...
    Query params:
        ...
        staff_email: Filter by staff member
...
@api_bp.route('/recordings/<recording_sid>/audio', methods=['GET'])
@api_or_session_auth
def get_recording_audio(recording_sid: str):
    """Stream recording audio file.

    Storage tiers checked in order:
    1. Local cache (3 weeks) - instant playback
    2. Google Drive (12 months) - fetch and re-cache locally
    3. Google Groups (forever) - not programmatically accessible

    Returns:
        Audio file (audio/mpeg) or 404 if not found
    """
    from rinq.services.recording_service import recording_service
    ...
    if file_path:
        # Local cache hit - update last_accessed_at and serve
    ...
    if drive_file_id:
        ...
        try:
            ...
            if audio_content:
                # Got the audio - save locally for future use
                ...
                # Serve the file
                return send_file(
                    recording_service.get_recording_file_path(recording_sid),
                    mimetype='audio/mpeg',
                    as_attachment=False,
                    download_name=f'{recording_sid}.mp3'
                )
    ...
    # 3. Google Groups is not programmatically accessible (cold archive only)
    # If we get here, the recording is either too old (purged from Drive)
    # or was never uploaded to Drive
    ...
    if google_message_id:
        return jsonify({
            "error": "Recording not in local cache or Drive. "
                     ...
                     "Contact support if you need access to old recordings."
        }), 404

    return jsonify({"error": "Recording not available - not stored in any accessible location"}), 404


@api_bp.route('/recordings/purge', methods=['POST'])
@api_or_session_auth
def purge_stale_recordings():
    """Purge local recording cache files not accessed recently.

    Storage tier: LOCAL (hot cache, 3 weeks default)

    Intended to be called by Skye on a schedule (e.g., weekly).
    Deletes local files but preserves Drive and Google Group archives.

    Body (JSON, optional):
        days: Number of days since last access (default 21 = 3 weeks)

    Returns:
        {"success": bool, "purged_count": N, "errors": [...]}
    """
    from rinq.services.recording_service import recording_service

    data = request.get_json() or {}
    days = data.get('days', 21)  # 3 weeks default
    ...
    status = 200 if result['success'] else 207  # 207 Multi-Status for partial success
    return jsonify(result), status


@api_bp.route('/recordings/purge-drive', methods=['POST'])
@api_or_session_auth
def purge_drive_recordings():
    """Purge old recordings from Google Drive.

    Storage tier: DRIVE (warm storage, 12 months default)

    Intended to be called by Skye on a schedule (e.g., monthly).
    Deletes from Drive but preserves Google Group archive (forever).

    Body (JSON, optional):
        days: Age threshold in days (default 365 = 12 months)

    Returns:
        {"success": bool, "deleted_count": N, "errors": [...]}
    """
    from rinq.services.drive_service import drive_service

    data = request.get_json() or {}
    days = data.get('days', 365)  # 12 months default
